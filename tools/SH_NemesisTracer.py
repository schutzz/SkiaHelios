...
# ============================================================
#  SH_NemesisTracer v1.2 [Seed Filter]
#  Mission: Trace the lifecycle of artifacts across USN/MFT.
#  Update: Added 'Generic Seed Blacklist' to prevent tracking noise.
# ============================================================
...
class NemesisTracer:
    def __init__(self, df_mft, df_usn, noise_validator_regex=None):
        ...
        # [NEW] 追跡してはいけない「ありふれたファイル名」リスト
        self.generic_seeds_blacklist = {
            "setup.exe", "install.exe", "update.exe", "installer.exe",
            "conhost.exe", "svchost.exe", "explorer.exe", "cmd.exe", "powershell.exe",
            "searchindexer.exe", "runtimebroker.exe", "backgroundtaskhost.exe",
            "desktop.ini", "thumbs.db", "index.dat", "ntuser.dat",
            "software", "system", "sam", "security", "default", # Registry hives often false flagged
            "unknown", "temp"
        }
    ...
    def _is_noise(self, text):
        ...
        # Regex check
        if self.noise_re and self.noise_re.search(str(text)): return True
        # [NEW] Generic Name check
        if str(text).lower() in self.generic_seeds_blacklist: return True
        return False
    ...
    def trace_lifecycle(self, attack_seeds):
        ...
        # ノイズ除去と小文字化。ここでも _is_noise を使うことでブラックリスト適用
        valid_seeds = {s.lower() for s in attack_seeds if len(s) > 2 and not self._is_noise(s)}
        if not valid_seeds: return []
        
        print(f"    -> [Nemesis] Tracing {len(valid_seeds)} unique seeds (Optimized Mode)...")

        lifecycle_events = []
        target_file_ids_map = {} 

        # 1. Seed Matching (High Speed Exact Match)
        for df, src in [(self.df_mft, "MFT"), (self.df_usn, "USN")]:
            ...
            try:
                filter_expr = pl.any_horizontal([
                    pl.col(c).str.to_lowercase().is_in(valid_seeds) for c in name_cols
                ])
                seed_hits = df.filter(filter_expr)

                for row in seed_hits.iter_rows(named=True):
                    # ヒットした行自体がノイズフォルダにある場合は除外
                    if self._is_noise(row.get("FileName")) or self._is_noise(row.get("ParentPath")): continue
                    
                    # [NEW] WinSxS等の構造的ノイズもここで再度チェック
                    ppath = str(row.get("ParentPath", "")).lower()
                    if "winsxs" in ppath or "msocache" in ppath: continue
                ...
                print(f"    [!] Seed Trace Error in {src}: {e}")

        # 2. ID Chain Recovery
        if target_file_ids_map:
    ...
    def trace_origin_by_execution(self, execution_events):
        ...
        for ev in execution_events:
            ...
            candidates = {c.lower() for c in candidates if len(c) > 2 and not self._is_noise(c)}
            ...
            for df in [self.df_usn, self.df_mft]:
                ...
                if time_col and name_cols:
                    try:
                        name_filter = pl.any_horizontal([
                            pl.col(c).str.to_lowercase().is_in(candidates) for c in name_cols
                        ])
                        ...
                        for row in hits.iter_rows(named=True):
                            ...
                            if "winsxs" in f_path.lower() or "msocache" in f_path.lower(): continue
        ...
        for df, src in [(self.df_usn, "USN"), (self.df_mft, "MFT")]:
            ...
            try:
                ...
                for row in chain_hits.iter_rows(named=True):
                    if self._is_noise(row.get("FileName")) or self._is_noise(row.get("ParentPath")): continue
                    if "winsxs" in str(row.get("ParentPath", "")).lower(): continue
    ...
    # ... (_parse_id, _extract_seeds_from_args, _to_event は変更なし) ...
